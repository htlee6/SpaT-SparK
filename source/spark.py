...
class SparK(nn.Module):
    def __init__(
            ...
            mask_ratio=0.6, densify_norm='bn', sbn=False, mask_strategy='random'
    ):
        super().__init__()
    ...
    def forward(self, inp_bchw: torch.Tensor, active_b1ff=None, vis=False, return_mode=''):
        # step1. Mask
        if active_b1ff is None:     # rand mask
            active_b1ff: torch.BoolTensor = self.mask(inp_bchw, inp_bchw.device)  # (B, 1, f, f)
